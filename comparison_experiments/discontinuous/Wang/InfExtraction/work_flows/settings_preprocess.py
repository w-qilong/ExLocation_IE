data_in_dir = "../../data/normal_data/traffic_remove_nested"
data_out_dir = "../../data/preprocessed_data/traffic"

# >>>>>>>>>>> used if "word_list" or "word2char_span" are provided >>>>>>>>>>>>>>>>>>>>>>>
word_tokenizer_type = "normal_chinese"  # white, stanza, normal_chinese;
language = "cn"  # used to init stanza for tokenizing, valid only if word_tokenizer_type = "stanza"

# >>>>>>>>>>>>>> bert >>>>>>>>>>>>>>>>>>
pretrained_model_tokenizer_path = "../../data/pretrained_models/bertbasechinese"
...
ignore_subword_match = False  # whether add whitespaces around the entities when searching spans, valid only if add_char_span = True
