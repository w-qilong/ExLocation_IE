...
class DatasetReader:
    def __init__(self, args):
        self.args = args
        self.parse = Parser() # 根据actions解析出mentions
    ...
    def read(self, filepath, training=False):
        ...
        with open(filepath, "r",encoding='utf-8') as f:
            for sentence in f:
                print(sentence)
                tokens = [Token(t) for t in sentence.strip().split()] # 句子对应的tokens
                annotations = next(f).strip()
                actions = self.parse.mention2actions(annotations, len(tokens)) # 将句子转换为actions
                oracle_mentions = [str(s) for s in self.parse.parse(actions, len(tokens))] # 根据actions，解析出mentions
                gold_mentions = annotations.split("|") if len(annotations) > 0 else [] # 标注的mentions列表：【【1,2 type】,【3,4 type】】
...
if __name__ == "__main__":
    ...
    vocab = Vocabulary.from_instances((instance for dataset in datasets.values() for instance in dataset)) # token2index and index2token
    ...
    with open(os.path.join(args.output_dir, "test.pred"), "w",encoding='utf-8') as f:
        for i in test_preds:
            f.write("%s\n%s\n\n" % (i[0], i[1]))
    ...
    if args.dev_filepath is not None:
        ...
        with open(os.path.join(args.output_dir, "dev.pred"), "w",encoding='utf-8') as f:
            for i in dev_preds:
                f.write("%s\n%s\n\n" % (i[0], i[1]))