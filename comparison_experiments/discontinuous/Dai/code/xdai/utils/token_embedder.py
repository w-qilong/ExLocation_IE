...
class TextFieldEmbedder(torch.nn.Module):
    ...
    def tokens_embedder(cls, vocab, args):
        ...
        weight = _load_pretrained_embeddings(args.pretrained_word_embeddings, dimension=50, token2idx=token2idx)
        return Embedding(len(token2idx), embedding_dim=50, weight=torch.FloatTensor(weight))
