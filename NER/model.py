...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # # 是否使用bert后四层
        # bert一共包含12层，有研究证明，模型由浅入深，各层提取句子的不同特征，浅层主要提取短语级别的表示，中间层主要包含语法信息，深层主要是
        # 词汇在句子中的语义表示，使用后四层的平均值，说明主要使用语义信息
        self.use_bert_last_4_layers = config.use_bert_last_4_layers
        self.tmp_config = config

        self.lstm_hid_size = config.lstm_hid_size # 512
        self.conv_hid_size = config.conv_hid_size # 96
        ...
        # 距离嵌入维度 距离值被设为1-19 dist_emb_size：20
        self.dis_embs = nn.Embedding(20, config.dist_emb_size)  # 位置嵌入
        # 区域编码被设置为三类， type_emb_size:20
        self.reg_embs = nn.Embedding(3, config.type_emb_size)  # 区域编码：区分上三角和下三角以及对角线
        # 输入维度768，输出维度256
        ...
        if config.use_dist_emb:
            conv_input_size = conv_input_size + config.dist_emb_size
        if config.use_type_emb:
            conv_input_size = conv_input_size + config.type_emb_size
    ...
    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length):
        ...
        dis_emb = self.dis_embs(dist_inputs) # distant input
        tril_mask = torch.tril(grid_mask2d.clone().long())
        reg_inputs = tril_mask + grid_mask2d.clone().long()
        reg_emb = self.reg_embs(reg_inputs) # rigion input

        if self.tmp_config.use_dist_emb and not self.tmp_config.use_type_emb:
            # print('only use use_dist_emb ')
            conv_inputs = torch.cat([dis_emb, cln], dim=-1)
        elif self.tmp_config.use_type_emb and not self.tmp_config.use_dist_emb:
            # print('only use use_type_emb ')
            conv_inputs = torch.cat([reg_emb, cln], dim=-1)
        else:
            # print('use use_type_emb and use_dist_emb')
