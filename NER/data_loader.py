...
def collate_fn(data):
    ...
    max_tok = np.max(sent_length) # batch句子最大长度
    sent_length = torch.LongTensor(sent_length) # 每个句子的长度
    max_pie = np.max([x.shape[0] for x in bert_inputs]) # pieces句子长度，包含了cls和sep
    bert_inputs = pad_sequence(bert_inputs, True) # 将句子填充到batch中最长句子的长度
...
def process_bert(data, tokenizer, vocab):
    '''
    :param data: 原始train，dev和test数据集
    :param tokenizer:
    :param vocab:
    :return:
    '''
    # 处理输入数据
    bert_inputs = [] # 每个句子对应的bert id列表
    grid_labels = [] # 每个句子的NNW和THW对应的矩阵
    grid_mask2d = [] # 和句子长度对应的值全为True的矩阵
    dist_inputs = [] # 句子中字之间的相对距离矩阵
    entity_text = [] # 句子中实体部分在句子中的index及类型组成的字符串的列表，如 [0-1-2-3-4-5-#-2]
    pieces2word = [] # bert分词后的pieces恢复到原本的词的矩阵
    sent_length = [] # 句子长度
    ...
    for index, instance in enumerate(data):
        ...
        tokens = [tokenizer.tokenize(word) for word in instance['sentence']]  # 对句子进行分词
        pieces = [piece for pieces in tokens for piece in pieces]  # 整个句子的pieces
        _bert_inputs = tokenizer.convert_tokens_to_ids(pieces)  # 句子转换为id
        # 加上CLS和SEP标签对应的id
        _bert_inputs = np.array([tokenizer.cls_token_id] + _bert_inputs + [tokenizer.sep_token_id])

        length = len(instance['sentence'])  # 句子的长度
        _grid_labels = np.zeros((length, length), dtype=np.int)  # 字与字之间NNW，THW标签的矩阵
        _pieces2word = np.zeros((length, len(_bert_inputs)), dtype=np.bool)  # pieces对应的标签矩阵
        _dist_inputs = np.zeros((length, length), dtype=np.int)  # 字之间的距离矩阵
        _grid_mask2d = np.ones((length, length), dtype=np.bool)  # mask矩阵
        ...
        if tokenizer is not None:
            ...
            for i, pieces in enumerate(tokens):
                if len(pieces) == 0:
                    continue
                pieces = list(range(start, start + len(pieces)))  # [0]
                _pieces2word[i, pieces[0] + 1:pieces[-1] + 2] = 1
                start += len(pieces)

        for k in range(length):  # 句子中字之间的相对位置矩阵，下面生成相对距离的方法很巧妙
            _dist_inputs[k, :] += k
            _dist_inputs[:, k] -= k
        ...
        # 生成NNW矩阵，上三角为NNW关系，下三角为THW-*关系
        for entity in instance["ner"]:
            index = entity["index"]
            for i in range(len(index)):
                if i + 1 >= len(index):
                    break
                _grid_labels[index[i], index[i + 1]] = 1 # 生成NNW关系
            _grid_labels[index[-1], index[0]] = vocab.label_to_id(entity["type"]) # 标记THW-*关系

        _entity_text = set([utils.convert_index_to_text(e["index"], vocab.label_to_id(e["type"]))
                            for e in instance["ner"]]) # 获得一个句子中的对应的实体，实体由index和type构成：如1-2-3-#-13
...
# instance:{'sentence':sentence, 'ner':[{"index": [14, 15, 16, 17], "type": "Expressway"}]}
...
def load_data_bert(config):
    ...
    vocab = Vocabulary()  # 存储所有实体类型
    # 统计各train、dev和test数据集中的实体数量
    ...
    config.label_num = len(vocab.label2id)  # label数量
    config.vocab = vocab  # label对应的词典集合


    ##*********
    train_dataset = RelationDataset(*process_bert(train_data, tokenizer, vocab))  # 将数据转换为tensor类型
